...
- (void)viewDidLoad {
  ...
  NSString *savedPowerValue = [[NSUserDefaults standardUserDefaults] stringForKey:@"UHF_RFID_POWER"];
  if ([savedPowerValue intValue]  > 0) {
    _inventoryResponder.outputPower = [savedPowerValue intValue];
  }
  sliderOutputPower.value = (savedPowerValue) ? [savedPowerValue intValue] : 15;
  lblSliderOutputPower.text = [NSString stringWithFormat:@"Output Power : %d", (savedPowerValue) ? [savedPowerValue intValue] :  15 ] ;
...
-(void)UHFRFIDInitialSetup{
  ...
  _inventoryResponder.transponderDataReceivedBlock = ^(TSLTransponderData *transponder, BOOL moreAvailable)
  {
    ...
    double absoluteValue = fabsl([transponder.rssi doubleValue]);
...
-(void)updateResults:(NSString *)data{
  if (data.length) {
    if ([UHFRFIDScannerResponseDelegate respondsToSelector:@selector(UHFRFIDScannerResponseSuccess:withType:withScanType:)]) {
      [UHFRFIDScannerResponseDelegate UHFRFIDScannerResponseSuccess:data withType: SCANNED_VALUE withScanType:@""];
    }
  }
}
 ...
 * Result get from the UHF RFID scanner
 */
-(void)updateBarcodeResults:(NSString *)data withScanType:(NSString *)scanType {
  if (data.length) {
    if ([UHFRFIDScannerResponseDelegate respondsToSelector:@selector(UHFRFIDScannerResponseSuccess:withType:withScanType:)]) {
      [UHFRFIDScannerResponseDelegate UHFRFIDScannerResponseSuccess:data withType: SCANNED_VALUE withScanType: SCAN_TYPE];
    }
  }
}
...
-(void)barcodeReceived:(NSString *)data {
  [self updateBarcodeResults: data withScanType:@"BARCODE"];
}
...
-(void)configureUserInventoryParameters:(TSLInventoryCommand *)command {
  ...
  if (value > 0 ) {
      [[NSUserDefaults standardUserDefaults] setObject:[NSString stringWithFormat:@"%.00f", sliderOutputPower.value] forKey:@"UHF_RFID_POWER"];
